...
export default function ClientDashboard() {
  ...
  const checkAndAwardBadges = async () => {
    ...
    if (!existingBadges.includes("streak_30") && user.current_streak >= 30) {
      newBadges.push({
        badge_type: "streak_30",
        badge_name: "Mese Inarrestabile",
        badge_description: "30 giorni di streak consecutivi",
        points_earned: 1000
      });
    }
    ...
    if (!existingBadges.includes("event_participant") && user.events_attended >= 1) {
      newBadges.push({
    ...
    if (!existingBadges.includes("ai_explorer")) {
      // Check if user has any workout plans generated by AI
      const workoutPlans = await base44.entities.WorkoutPlan.filter({ user_email: user.email });
    ...
    if (!existingBadges.includes("gym_enthusiast")) {
      // Check if user has visited 5 different gyms
      const gymAccesses = await base44.entities.GymAccess.filter({ user_email: user.email });
      const uniqueGyms = new Set(gymAccesses.map(a => a.gym_id)).size;
      if (uniqueGyms >= 5) {
        newBadges.push({
          badge_type: "gym_enthusiast",
          badge_name: "Amante delle Palestre",
          badge_description: "Visita 5 palestre diverse",
          points_earned: 200
        });
      }
    }
